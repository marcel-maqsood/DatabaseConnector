...
class PersistentPDO
{
	...
	/**
	 * fetchSql() expects a sql string as first argument and prepares, executes and returns just that.
	 ...
	 * @param array $params - params that might be used within the sql
	 * @param bool $debug - Wether or not the functiion should dump the current sql for debugging.
	 *
	 * @return array|null - The result array of the query.
	 */
	public function fetchSql($sql, $params = [], $debug = false)
	{
		if ($debug) {
			var_dump($sql, $params);
			exit;
	...
	/**
	 * getAllBase() expects a sql string as first argument and prepares, executes it and returns it as edited array.
	 * This function will always return an array with all fields that got found with given conditions or NULL.
	 *
	 * @param string $sql - The SQL Statement that should be executed
	 * @param bool $debug - Wether or not the functiion should dump the current sql for debugging.
